...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    ...
    this.filterAcc(tw => !(that.contains(tw)), that)
  }
  def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty) {
      if (right.mostRetweeted.retweets < elem.retweets) elem
      else right.mostRetweeted
    }
    else if (right.isEmpty){
      if (left.mostRetweeted.retweets < elem.retweets) elem
...
object GoogleVsApple {
  ...
  //  (tweet: Tweet) => google.exists((word: String) => tweet.text.contains(word))
  //)
  //lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(
  //  (tweet: Tweet) => apple.exists((word: String) => tweet.text.contains(word))
  //)
